...
void messageReceived(String &topic, String &payload) {
  ...
  //verification la statue du rouge
  if(payload.charAt((payload.indexOf(':') + 5)) == 'R' ){
  statusR = payload.substring(payload.lastIndexOf(':') + 1 ,payload.indexOf('}'));
  }
  
  //verification la statue du Vert
  if(payload.charAt((payload.indexOf(':') + 5)) == 'G' ){
  statusG = payload.substring(payload.lastIndexOf(':') + 1 ,payload.indexOf('}'));
  }

  //verification la statue du bleu
  if(payload.charAt((payload.indexOf(':') + 5)) == 'B' ){
  statusB = payload.substring(payload.lastIndexOf(':') + 1 ,payload.indexOf('}'));
  }

  //intensitee de LED
  if(payload.charAt((payload.indexOf(':') + 11)) == 's' ){
    ledBrightFl = (payload.substring(payload.lastIndexOf(':') + 1 ,payload.indexOf('}'))).toInt();
  }
